...
def test_geo_on_missing_warn(caplog):
    draft = "print([[GEO:point:A]])"
    with caplog.at_level(logging.WARNING):
        out = fill_placeholders(draft, [], on_missing="warn_token")
    assert out.manim_code_final == draft
    assert "Unreplaced GEO placeholder" in caplog.text


def test_geo_on_missing_fail():
    draft = "print([[GEO:point:A]])"
    with pytest.raises(ValueError):
        fill_placeholders(draft, [], on_missing="fail_build")
