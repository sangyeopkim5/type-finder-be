...
function Header({ onAddNode }) {
  ...
  const [setName, setSetName] = useState('Manion-1');
  ...
  const nodeTypes = [
    { type: 'basic', label: '노드', description: '', icon: 'basic' },
    { type: 'concept', label: '개념 심화 노드', description: '개념 심화, 증명, 연관개념', icon: 'lightbulb' },
    { type: 'strategy', label: '전략 수립 노드', description: '풀이전략 & 스토리보드', icon: 'compass' },
    { type: 'visualization', label: '시각화 노드', description: '그래프, 이미지, 3D 도형', icon: 'chart' },
    { type: 'video', label: '영상화 노드', description: '시각화, 풀이 영상', icon: 'video' },
    { type: 'summary', label: '요약 노드', description: '오디오, Text요약', icon: 'microphone' }
  ];

  const versions = [
    { id: 1, name: 'V1.버전 이름', subtitle: '버전 부제목(내용)' },
    { id: 2, name: 'V2.버전 이름', subtitle: '버전 부제목(내용)' },
    { id: 3, name: 'V3.버전 이름', subtitle: '버전 부제목(내용)' }
  ];
  ...
  const renderIcon = (iconType) => {
    switch (iconType) {
      case 'basic':
        return (
          <svg width="50" height="47" viewBox="0 0 50 47" fill="none" className="magnifying-glass">
              <circle cx="20" cy="20" r="12" stroke="black" strokeWidth="3"/>
              <path d="M28 28L38 38" stroke="black" strokeWidth="3" strokeLinecap="round"/>
          </svg>
        );
      case 'lightbulb':
        return (
          <svg width="50" height="45" viewBox="0 0 55 54" fill="none" className="lightbulb">
      ...
      case 'compass':
        return (
          <svg width="46" height="46" viewBox="0 0 54 54" fill="none" className="compass-icon">
      ...
      <div className="sidebar-content">
        ...
        {isEditingName ? (
          ...
          <div className="set-name-display" onClick={() => setIsEditingName(true)}>
            ...
            <div className="set-name-edit-icon">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3H5C4.44772 3 4 3.44772 4 4V7M8 3V7M8 3H11C11.5523 3 12 3.44772 12 4V7M8 7H4M8 7H12M8 7V11M4 7V11M12 7V11M8 11H4M8 11H12" stroke="black" strokeWidth="1.5"/></svg>
            </div>
          </div>
        )}
        ...
        {nodeTypes.map(node => (
          <div className="node-item" key={node.type}>
            <div className={`node-icon ${node.type}-node`}>
              {renderIcon(node.icon)}
            </div>
            <div className="node-info">
              <div className="node-title">{node.label}</div>
              {node.description && <div className="node-description">{node.description}</div>}
            </div>
        ...
        {versions.map(version => (
          <div className="version-item" key={version.id}>
            ...
            <button className="restore-button">복원</button>
          </div>
        ))}
        ...
        <div className="action-buttons">
          <button className="action-btn">
            <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M17 3H2V17H17V3Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M6 7H13M6 11H13M6 15H11" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span>저장</span>
          </button>
          <button className="action-btn">
            <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M1 9.5H17M9.5 1V18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span>공유하기</span>
          </button>
          <button className="action-btn">
            <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="9.5" cy="9.5" r="8" stroke="currentColor" strokeWidth="2"/>
              <path d="M9.5 5V9.5L12.5 12.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span>초기화</span>
          </button>
          <button className="action-btn">
            <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="9.5" cy="6.5" r="2.5" stroke="currentColor" strokeWidth="2"/>
              <path d="M2 17.5C2 14.4624 5.46243 12 9.5 12C13.5376 12 17 14.4624 17 17.5" stroke="currentColor" strokeWidth="2"/>
            </svg>
            <span>계정</span>
          </button>
        </div>
